...
namespace ft
{
	...
	public:
			...
			ft::enable_if<!ft::is_integral<InputIterator>::value>::type* = NULL)
		{
			...
			this->_finish = this->_start;
			this->_end_of_storage = this->_start + n;
			for (; first != last; first++) this->_alloc.construct(this->_finish++, *first);
		}
			...
			: _alloc(other._alloc)
		{
			...
			this->_finish = this->_start;
			this->_end_of_storage = this->_start + n;
			for (iterator it = other.begin(); it != other.end(); it++) this->_alloc.allocate(this->_finish++, *it);
		}
		...
		*/
		...
		void reserve(size_type n)
		{
			...
			if (this->capacity() > n)
				return;
			const size_type old_capacity = this->_size();
